...
class _HistoryState extends State<History> {
  ...
  List<Map<String, dynamic>> _rolesPast = [];
  ...
  Future cargarData() async {
    ...
    await prefs.setString('last_nav_position', "history");
    ...
    //todos los roles pasados
    final data = await SQLHelper.getPastRoles();
    _rolesPast = data;

    for (int i = 0; i < _rolesPast.length; i++) {
      events.add(
        CalendarEvent(
          eventID: _rolesPast[i]['rolmaestroid'].toString(),
          eventName: _rolesPast[i]['nombretienda'],
          eventDate: DateTime.parse(_rolesPast[i]['fechaprogramadavisitarol'] +
              " " +
              _rolesPast[i]['horaprogramadavisitarol']),
          eventBackgroundColor: Color(0xFF129fd6),
